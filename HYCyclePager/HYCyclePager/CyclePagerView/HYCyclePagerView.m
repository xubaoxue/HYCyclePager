...
NS_INLINE BOOL HYEqualIndexSection(HYIndexSection indexSection1,HYIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE HYIndexSection HYMakeIndexSection(NSInteger index, NSInteger section) {
    HYIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface HYCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, HYCyclePagerTransformLayoutDelegate> {
    struct {
...
@property (nonatomic, assign) HYIndexSection    beginDragIndexSection;
...
@implementation HYCyclePagerView
...
- (void)addCollectionView {
    HYCyclePagerLayout *layout = [[HYCyclePagerLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:HYPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (HYCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<HYCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((HYCyclePagerLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<HYCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(HYPagerScrollDirection)direction animate:(BOOL)animate {
    HYIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    ...
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:HYMakeIndexSection(index, 0) animate:animate];
        return;
    }

    [self scrollToItemAtIndexSection:HYMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:animate];
}
...
- (void)updateLayout {
    ...
    ((HYCyclePagerLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(HYIndexSection)indexSection {
...
- (HYIndexSection)nearlyIndexPathAtDirection:(HYPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (HYIndexSection)nearlyIndexPathForIndexSection:(HYIndexSection)indexSection direction:(HYPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == HYPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? HYMakeIndexSection(0, 0) : indexSection;
        } else if (direction == HYPagerScrollDirectionRight) {
            return HYMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? HYMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return HYMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == HYPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return HYMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount-1) {
            return HYMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount-1);
        }
        return HYMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return HYMakeIndexSection(indexSection.index, 0);
    }
    return HYMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (HYIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return HYMakeIndexSection(0, 0);
    }
    ...
    return HYMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(HYIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:HYMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animate:NO];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_delegate respondsToSelector:@selector(pagerView:didSelectedItemCell:atIndexSection:)]) {
        [_delegate pagerView:self didSelectedItemCell:cell atIndexSection:HYMakeIndexSection(indexPath.item, indexPath.section)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    HYIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    HYIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !HYEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (fabs(velocity.x) < 0.35 || !HYEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    HYPagerScrollDirection direction = HYPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = HYPagerScrollDirectionLeft;
    }
    HYIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - HYCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((HYCyclePagerLayout *)_collectionView.collectionViewLayout).delegate = nil;
