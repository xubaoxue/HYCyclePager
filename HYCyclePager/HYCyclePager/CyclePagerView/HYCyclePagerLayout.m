...
typedef NS_ENUM(NSUInteger, HYTransformLayoutItemDirection) {
    HYTransformLayoutItemLeft,
    HYTransformLayoutItemCenter,
    HYTransformLayoutItemRight,
};


@interface HYCyclePagerLayout () {
    struct {
...
@interface HYCyclePagerViewLayout ()
...
@implementation HYCyclePagerLayout
...
- (void)setDelegate:(id<HYCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(HYCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (HYTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    HYTransformLayoutItemDirection direction= HYTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = HYTransformLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = HYTransformLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == HYCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != HYCyclePagerTransformLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(_layout.layoutType != HYCyclePagerTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(HYCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case HYCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case HYCyclePagerTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(HYCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case HYCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        HYTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case HYTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case HYTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation HYCyclePagerViewLayout
